    ...
    try {
        ...
        try {
            ...
            for (const key of orderItems) {
                ...
                let name = await pool.query("SELECT description FROM product WHERE EAN = $1", [key.EAN]);
                name = name.rows[0].description;
                let price = await pool.query("SELECT priceHistory.gross FROM priceHistory JOIN product ON priceHistory.product_fk = product.product_pk WHERE ((product.EAN = $1) AND (((priceHistory.startDate <= CURRENT_TIMESTAMP) AND (priceHistory.endDate IS NULL)) OR ((priceHistory.startDate <= CURRENT_TIMESTAMP) AND (priceHistory.endDate > CURRENT_TIMESTAMP))));", [key.EAN]);
                price = parseInt(price.rows[0].gross * 100);
        ...
        const newOrder = await pool.query("INSERT INTO orders (order_id, isSale) VALUES($1, $2) RETURNING *", [order_id, isSale]);
        ...
        for (const key of orderItems) {
            ...
            var product_fk = await pool.query("SELECT product_pk FROM product WHERE EAN = $1", [key.EAN]);
            product_fk = product_fk.rows[0].product_pk;
            //Create the new order item
            await pool.query("INSERT INTO order_item (order_fk, product_fk, quantity, percentageModifier) VALUES($1, $2, $3, $4) RETURNING *", [order_pk, product_fk, key.Quantity, key.percentageModifier]);
        }
        
        //Push the payment link and order_id to the return data
        returnData.push({
...
app.post("/search", async (req, res, next) => {
    try {
        ...
        // Search for products
        const products = await pool.query("SELECT * FROM product WHERE description LIKE $1", [search]);
...
app.post("/getEanInfo", async (req, res, next) => {
    try {
        ...
        // Get product name and price from database
        let name = await pool.query("SELECT description FROM product WHERE EAN = $1", [EAN]);
        name = name.rows[0].description;
        let price = await pool.query("SELECT priceHistory.gross FROM priceHistory JOIN product ON priceHistory.product_fk = product.product_pk WHERE ((product.EAN = $1) AND (((priceHistory.startDate <= CURRENT_TIMESTAMP) AND (priceHistory.endDate IS NULL)) OR ((priceHistory.startDate <= CURRENT_TIMESTAMP) AND (priceHistory.endDate > CURRENT_TIMESTAMP))));", [EAN]);
        price = price.rows[0].gross;
        let stock = await pool.query("SELECT stock FROM product WHERE EAN = $1", [EAN]);
...
app.post("/getOrderInfo", async (req, res, next) => {
    try {
        ...
        let order = await pool.query("SELECT * FROM orders WHERE order_id = $1", [order_id]);
        order = order.rows[0];
        // Get order time from database
        let orderTime = await pool.query("SELECT orderTime FROM orders WHERE order_id = $1", [order_id]);
        ...
        let orderItems = await pool.query("SELECT * FROM order_item WHERE order_fk = $1", [order.order_pk]);
        for (const key of orderItems.rows) {
            // Get product name
            let name = await pool.query("SELECT * FROM product WHERE product_pk = $1", [key.product_fk]);
            name = name.rows[0].description;

            // Get price from database at the time of the order
            let price = await pool.query("SELECT priceHistory.gross, priceHistory.endDate FROM priceHistory JOIN product ON priceHistory.product_fk = product.product_pk WHERE ((product.product_pk = $1) AND (((priceHistory.startDate <= $2) AND (priceHistory.endDate IS NULL)) OR ((priceHistory.startDate <= $2) AND (priceHistory.endDate > $2))));", [key.product_fk, orderTime]);
            price = price.rows[0].gross;
